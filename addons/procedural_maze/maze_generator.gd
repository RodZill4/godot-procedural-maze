...
func open(rect):
	...
	print("opening zone "+str(rect)+" -> (%d, %d)-(%d, %d)" % [min_x, min_y, max_x, max_y])
...
func generate_floor(maze, material):
	...
	# Generate mesh
	if material != null:
		var context = { vertices=[ ], uvs=[ ], normals=[ ], tangents=[ ] }
		...
		mesh.mesh.add_surface_from_arrays(ArrayMesh.PRIMITIVE_TRIANGLE_STRIP, [ PoolVector3Array(context.vertices), PoolVector3Array(context.normals), PoolRealArray(context.tangents), null, PoolVector2Array(context.uvs), null, null, null, null ])
...
func generate_walls(maze, material):
	...
	mesh.name = "walls"
	mesh.mesh = ArrayMesh.new()
	mesh.mesh.add_surface_from_arrays(ArrayMesh.PRIMITIVE_TRIANGLES, [ PoolVector3Array(context.vertices), PoolVector3Array(context.normals), PoolRealArray(context.tangents), null, PoolVector2Array(context.uvs), null, null, null, null ])
	mesh.mesh.surface_set_material(0, material)
	maze.add_child(mesh)
